...
t_acao_qtd = get_table('### 2. Ações vs Não-Ações por Ano (Quantidade %)')
t_acao_mcap = get_table('### 3. Ações vs Não-Ações por Ano (Market Cap %)')
t_ns_qtd = get_table('### 4. Detalhe de Não-Ações por Ano (Quantidade %)')
t_ns_mcap = get_table('### 5. Detalhe de Não-Ações por Ano (Market Cap %)')
t_setor_qtd = get_table('### 6. Ações por Setor e Ano (Quantidade %)')
t_setor_mcap = get_table('### 7. Ações por Setor e Ano (Market Cap %)')
t_hcm_extremos = get_table('### 8. HCM: Decil 1 (Central) vs Decil 10 (Periférico) - Tipos de Ativo (Count %)')
t_pozzi_extremos = get_table('### 9. Pozzi: Decil 1 (Central) vs Decil 10 (Periférico) - Tipos de Ativo (Count %)')
t_hcm_setores_extremos = get_table('### 10. HCM: Setores nas Ações - Decil 1 vs Decil 10 (Count %)')
