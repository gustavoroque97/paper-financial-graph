...
for year in years:
    ...
    q10 = active_with_mcap['Market Cap'].quantile(0.10)
    q90 = active_with_mcap['Market Cap'].quantile(0.90)
    
    small_tickers = active_with_mcap[active_with_mcap['Market Cap'] <= q10]['Ticker'].tolist()
    big_tickers = active_with_mcap[active_with_mcap['Market Cap'] >= q90]['Ticker'].tolist()
...
def calculate_metrics(returns_simple, series_name):
    ...
    return pd.Series({"CAGR": cagr, "Vol": vol_annual, "CumRetSmall": (1+np.expm1(serie_small)).cumprod().iloc[-1], "CumRetBig": (1+np.expm1(serie_big)).cumprod().iloc[-1]}, name=series_name)

print(calculate_metrics(smb_simple, "SMB q10-q90"))
